...
for seed in seeds:
    ...
    for episode in range(FLAGS.nepisodes):
        ...
        if newpos is None and newobs not in walls:
            ...
            # ax.imshow(pos.reshape(env.occupancy.shape),cmap=new_map)                
            # plt.show()
            # plt.close()
        ...
        for step in range(FLAGS.nsteps):
            ...
            # if step>0 and plotsteps:
            # if (done or step==FLAGS.nsteps-1) and plotsteps:
            ...
            # if init_set and step>0 and plotsteps:
                ...
                #     pol_values = option_policies[0].weights[:,i].copy()
                ...
                #     plan = env.occupancy.flatten().astype(float)
                #     pol_values = option_policies[0].weights[:,i].copy()
                #     plan[plan == 1] = min(pol_values) - 0.01
                #     plan[plan == 0] = pol_values                 
            ...
            #     ax[0].imshow(pos.reshape(env.occupancy.shape),cmap=new_map)  
            #     ax[1].imshow(critic_map1.reshape(env.occupancy.shape),cmap=new_map)              
            #     plt.show()
            #     plt.close()                
            #     pdb.set_trace()
            ...
            if option ==1 and False:
                ...
                if np.random.rand() < myrand:
                    ...
                    action = option_policies[option].sample(phi,step)
    ...
    # pdb.set_trace()
        ...
        #     np.savetxt("{}_graph_seed{}.csv".format(row*col,seed),totalsteps,delimiter=',')       
        # elif episode%100==0:

        #     np.savetxt("{}_nograph_seed{}.csv".format(row*col,seed),totalsteps,delimiter=',')       
