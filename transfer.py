...
def simfour(seed):
    ...
    parser.add_argument('--nepisodes', help="Number of episodes per run", type=int, default=8)
    ...
    parser.add_argument('--app', help='dont worry', type=str, default='')
    parser.add_argument('--f', help='be happy', type=int, default=1)
    parser.add_argument('--nf', help='its just a hack', type=int, default=1)
    ...
    for run in range(args.nruns):
        ...
        for episode in range(args.nepisodes):
            ...
            for step in range(args.nsteps):
                ...
                if observation != next_observation:
                    G.add_edge(full_grid_dict.get(observation),full_grid_dict.get(next_observation))
                observation=next_observation

                phi = features(observation)
                allobs.add(full_grid_dict.get(observation))
                # pdb.set_trace()
        ...
        #     xtra_feats.append(interpol(w)[0])
        # critic_map = env.occupancy.flatten().astype(float)
        # critic_vals = critic.weights[:,0].copy()
        ...
        features[env.goal] = 1. # little hack
        # pdb.set_trace()
